...
$(document).ready(async function () {
    try {
        ...
        // Populate the <select> element with the country names
        const selectElement = $("#selectCountries");

        countries.forEach(country => {
            selectElement.append(`<option value="${country.country_name}">${country.country_name}</option>`);
        });
        ...
        selectElement.on('change', async function () {
            ...
            const states = await getStatesForCountry(accessToken, selectedCountry);

            // Populate the <select> element with the states
            const selectStatesElement = $("#selectStates");
            selectStatesElement.empty(); // Clear existing options
            states.forEach(state => {
                selectStatesElement.append(`<option value="${state.state_name}">${state.state_name}</option>`);
            });
        });

        const selectStatesElement = $("#selectStates");

        selectStatesElement.on('change', async function () {
            // Get the selected state name
            const selectedstate = $(this).val();

            // Fetch the list of states for the selected country
            const cities = await getcitiesForstate(accessToken, selectedstate);

            // Populate the <select> element with the states
            const selectcitiesElement = $("#selectcity");
            selectcitiesElement.empty(); // Clear existing options
            cities.forEach(city => {
                selectcitiesElement.append(`<option value="${city.city_name}">${city.city_name}</option>`);
            });
        });
...
async function getAccessToken() {
    const response = await $.ajax({
        ...
        headers: {
            ...
            "api-token": "xzyV_cOJT5BwTzQZ2MZLW6owLKrEHxQ2CcBTjgN25xo-FGrnvxX8PHSH4YYcBnkKCDM",
            "user-email": "aliaadil107@gmail.com"
        }
    });
    return response.auth_token;
}

// Function to get the list of countries using the access token
async function getCountries(accessToken) {
    const response = await $.ajax({
...
// Function to get the list of states for a specific country using the access token
async function getStatesForCountry(accessToken, countryName) {
    const response = await $.ajax({
...
async function getcitiesForstate(accessToken, selectedstate) {
    const response = await $.ajax({
